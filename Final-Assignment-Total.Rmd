...
left which could be explained by a ceiling effect. Furthermore, for all
...
only one chain, we can not use the Gelman-Rubin statistic.
...
the more skewed a distribution is. 

To investigate whether the original statistic works, we will compare the results 
to the known statistic of skewness:
...
of the residuals. Skewness is known to measure asymmetry of a distribution. Therefore, we will use it 
...
second_ppp <- sum(sim_skew > obs_skew)/1000
second_ppp
```
