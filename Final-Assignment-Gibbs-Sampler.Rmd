...
GibbsSampler <- function(Y, X1, X2, burn.in = 1000, iterations = 10000, initial.values = c(1, 1, 1), method = c("0", "0", "0")) {
  ...
  for(i in 2:iterations) {
    ...
    if(method[1] == "0") {
      ...
      mean.0.post <- (sum(Y - (b1[i - 1]*X1 - b2[i - 1]*X2), na.rm = TRUE)/vari[i - 1] + (mu0 / sigma0)) / ((N / vari[i - 1]) + (1 / sigma0))
      ...
      accept_ratio <- (dnorm(proposal_value)/dnorm(b0[i - 1])) * (proposal(b0[i - 1])/proposal(proposal_value))
    ...
    if(method[2] == "0") {
      ...
      mean.1.post <- ((sum(X1*(Y - b0[i] - b2[i - 1]*X2), na.rm = TRUE)/vari[i - 1]) + (mu1 / sigma1)) / ((sum((X1^2), na.rm = TRUE)/vari[i - 1]) + (1 / sigma1))
